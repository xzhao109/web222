 ...
 function greeting(name) {
  ...
  function fixPostalCode(postalCode) {
    ...
    if (postalCode.length !== 7) {
      ...
      //  non-digit in 2th 5th 7th position
      if (isNaN(postalCode[1]) || isNaN(postalCode[4]) || isNaN(postalCode[6])) {
        throw postalCode;
      }
      //  non-space in 4th position
      if (postalCode[3] !== ' ') {
        throw postalCode;
      }
      //  invalid character in 3th 5th position
  ...
  function toProvince(postalCode, useShortForm) {
    ...
    // invalid postal code return 'null'
    // try {
    //   fixPostalCode(postalCode).toThrow();
    // } catch(err) {
    //   return null;
